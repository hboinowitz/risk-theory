...
for (day in (1: t)) {
  ...
  daily_costs_reinsurance_quota <- c(
    daily_costs_reinsurance_quota, costs_reinsurrance_quota
  )
}
...
for (day in (1: t)) {
  ...
  cases_reinsurance_xl <- c(cases_reinsurance_xl, N_reinsurrance_xl) 
  daily_costs_reinsurance_xl <- c(
    daily_costs_reinsurance_xl, costs_reinsurrance_xl
  )
}
...
total_costs_reinsurance_stop_loss <- max(total_costs - cap_stop_loss, 0)
...
for (day in (1: t)) {
  costs <- costs_per_day[[day]]
  costs_reinsurrance_ecomor <- sum(mapply(\(x) max(x - M_ecomor, 0), costs))
  N_reinsurrance_ecomor <- sum(as.integer(costs > M_ecomor))
  cases_reinsurance_ecomor <- c(cases_reinsurance_ecomor, N_reinsurrance_ecomor) 
  daily_costs_reinsurance_ecomor <- c(
    daily_costs_reinsurance_ecomor, costs_reinsurrance_ecomor
  )
}
...
matplot(cumulated_costs, type = c("b"), pch=1, col = 1:3, xlab="t [d]", ylab="Costs RI [€]")
legend("topleft", legend = c("ECOMOR", "Quota", "XL"), col=1:4, pch=1) # optional legend

matplot(daily_costs_plot, type = c("b"), pch=1, col = 1:3, xlab="t [d]", ylab="Costs RI [€]")
legend("topleft", legend = c("ECOMOR", "Quota", "XL"), col=1:4, pch=1) # optional legend
